...
For each time-frequency bin, the current implementation constructs a 15-dimensional feature vector:
...
where:
- \(z_f(t,f)\) is the frequency-wise normalized log-power score,
- \(z_t(t,f)\) is the time-wise normalized log-power score,
- \(\mu_{3\times3}\), \(\sigma_{3\times3}\), \(\mu_{5\times5}\), and \(\sigma_{5\times5}\) are local mean and standard deviation features,
- \(c_{3\times3}\) and \(c_{5\times5}\) are local contrast features,
- \(\nabla_f L\) and \(\nabla_t L\) are frequency- and time-direction gradients,
- \(|\nabla_f L|\) and \(|\nabla_t L|\) are absolute gradient magnitudes,
...
- threshold selection: validation-set search using mean IoU as the primary metric
- threshold search grid: `0.40, 0.45, 0.50, 0.55`
- selected threshold: chosen from the validation split and stored in the saved model metadata
- evaluation split: held-out FUSS test split
- evaluated examples: depends on the available valid FUSS examples and the train/validation/test split
...
### Threshold tuning
A fixed probability threshold is selected on the validation set by maximizing mean IoU over the threshold grid. This avoids tuning the threshold directly on the test set and makes the final test evaluation more meaningful.
...
The current FUSS benchmark code uses:
- training / validation / test split
- validation-based threshold selection
- held-out test evaluation
- `NUM_EXAMPLES = None` by default, meaning all valid foreground examples are used
- the number of evaluated test examples depends on the available valid examples after splitting
---
...
### Threshold search
The candidate thresholds `0.40`, `0.45`, `0.50`, and `0.55` were tested on the validation set. The best threshold was selected using mean IoU as the primary validation metric before final test evaluation.

...
FUSS_RESULTS_DIR = ensure_dir(get_config_path(config, "fuss_results_folder"))
...
if NUM_EXAMPLES is None:
    MODEL_PKL = MODELS_DIR / "fuss_ml_lgbm_crf_model_all.pkl"
    OUT_CSV = FUSS_RESULTS_DIR / "fuss_ml_lgbm_crf_metrics_all.csv"
    TEST_LIST = FUSS_OUTPUTS_DIR / "fuss_test_files.txt"
else:
    MODEL_PKL = MODELS_DIR / f"fuss_ml_lgbm_crf_model_seed{SEED}_{NUM_EXAMPLES}.pkl"
    OUT_CSV = FUSS_RESULTS_DIR / f"fuss_ml_lgbm_crf_metrics_seed{SEED}_{NUM_EXAMPLES}.csv"
    TEST_LIST = FUSS_OUTPUTS_DIR / f"fuss_test_files_seed{SEED}_{NUM_EXAMPLES}.txt"
...
def build_sampled_dataset(file_list, rng):
    ...
    for i, mix_path in enumerate(file_list, start=1):
        ...
        try:
            X_full, Sxx, _, _ = ml.extract_feature_matrix_file(
                mix_path,
                nperseg=NPERSEG,
                noverlap=NOVERLAP,
...
def evaluate_files(file_list, model, threshold):
    ...
    for i, mix_path in enumerate(file_list, start=1):
        ...
        try:
            pred_mask, raw_prob, decoded_prob, Sxx = ml.estimate_mask_file(
                mix_path,
                model=model,
                nperseg=NPERSEG,
                noverlap=NOVERLAP,
...
def main():
    ...
    if REUSE_SAVED_MODEL and MODEL_PKL.exists():
        ...
        print("\nTraining LightGBM...")
        model = ml.train_lgbm_classifier(
            X_train,
            y_train,
            X_val=X_val,
            y_val=y_val,
    ...
    summary = summarize_rows(test_rows, title="LightGBM + 2D CRF on FUSS (test split)")

"""
Evaluate Method 5: Hybrid DSP-DL U-Net on FUSS.
...
OUT_CSV = FUSS_RESULTS_DIR / "fuss_hybrid_dsp_dl_metrics.csv"
...
THRESHOLD_GRID = None        # e.g. [0.40, 0.45, 0.50, 0.55] to search on this split
...
def evaluate_at_threshold(model, files: List[pathlib.Path], threshold: float):
    ...
    for i, mix_path in enumerate(files, start=1):
        ...
        mask, prob, dsp_prob, _ = hdl.estimate_mask_file(
            mix_path,
            model=model,
            nperseg=NPERSEG,
            noverlap=NOVERLAP,
...
def main():
    ...
    if THRESHOLD_GRID is not None:
        ...
        for threshold in THRESHOLD_GRID:
            ...
            if best_summary is None or summary["iou"] > best_summary["iou"]:

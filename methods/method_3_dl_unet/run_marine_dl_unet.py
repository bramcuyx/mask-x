"""
Run Method 3: Pure DL Spectrogram U-Net on marine event wav files.

This script generates:
  - binary masks:   masks_folder/dl_unet/*.npy
  - probabilities:  masks_folder/dl_unet_prob/*.npy
  - plots:          plots_folder/dl_unet/*.png
...
def save_prediction_figure(path, Sxx, mask, prob, f, t):
    ...
    ax[1].set_title("U-Net Probability")
...
def main():
    ...
    checkpoint_path = get_config_path(config, "dl_unet_checkpoint")
    ...
    masks_folder = marine_outputs_dir / "masks" / "dl_unet"
    prob_folder = marine_outputs_dir / "probabilities" / "dl_unet"
    plots_folder = marine_outputs_dir / "plots" / "dl_unet"
    ...
    for i, file in enumerate(event_files, start=1):
        ...
        mask, prob, Sxx, f, t = dl.estimate_mask_file(
            file,
            model=model,
            nperseg=nperseg,
            noverlap=noverlap,
        ...
        mask, cleaned = enforce_marine_padding(
            ...
            t,
            padding=padding,
            probability_maps=[prob],
        )

        prob = cleaned[0]

        f_plot, t_plot, Sxx_plot = compute_plot_spectrogram(file, resampled_sr, nperseg, noverlap)
        Sxx_plot, f_plot, t_plot, cropped = crop_to_common_grid(Sxx_plot, f_plot, t_plot, mask, prob)
        mask_plot, prob_plot = cropped

        np.save(masks_folder / f"{file.stem}.npy", mask.astype(bool))
        np.save(prob_folder / f"{file.stem}.npy", prob.astype(np.float32))
        save_prediction_figure(plots_folder / f"{file.stem}.png", Sxx_plot, mask_plot, prob_plot, f_plot, t_plot)

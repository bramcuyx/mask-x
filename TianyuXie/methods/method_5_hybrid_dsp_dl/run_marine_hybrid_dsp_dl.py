...
CHECKPOINT_PATH = pathlib.Path(__file__).with_name("models") / "fuss_hybrid_dsp_dl_best.pt"
...
def main():
    ...
    masks_folder = pathlib.Path(config["paths"]["masks_folder"]) / "hybrid_dsp_dl"
    prob_folder = pathlib.Path(config["paths"]["masks_folder"]) / "hybrid_dsp_dl_prob"
    plots_folder = pathlib.Path(config["paths"]["plots_folder"]) / "hybrid_dsp_dl"

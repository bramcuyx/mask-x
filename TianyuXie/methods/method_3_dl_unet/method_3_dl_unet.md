...
A binary mask is obtained using a fixed threshold selected on the FUSS validation split:
...
Edit `ROOT` to the local FUSS folder, then run:
...
```text
models/fuss_dl_unet_metrics.csv
```
...
```text
masks_folder/dl_unet/*.npy
masks_folder/dl_unet_prob/*.npy
plots_folder/dl_unet/*.png
```

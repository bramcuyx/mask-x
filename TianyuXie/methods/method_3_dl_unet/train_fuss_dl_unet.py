"""
Train Method 3: Pure DL Spectrogram U-Net on FUSS.
...
OUT_DIR = pathlib.Path(__file__).with_name("models")
CHECKPOINT_PATH = OUT_DIR / "fuss_dl_unet_best.pt"
TRAIN_LOG_CSV = OUT_DIR / "fuss_dl_unet_training_log.csv"
...
class FussMaskDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        data, sr = dl.load_audio(mix_path, resampled_sr=RESAMPLED_SR)
        _, _, Sxx = dl.compute_spectrogram(data, sr, nperseg=NPERSEG, noverlap=NOVERLAP)
        log_sxx = dl.to_log_spectrogram(Sxx)
        x, _, _ = dl.normalize_log_spectrogram(log_sxx)
        y = compute_truth_mask(source_dir)

        nf = min(x.shape[0], y.shape[0])
        nt = min(x.shape[1], y.shape[1])
        x = x[:nf, :nt]
        y = y[:nf, :nt]
...
def run_one_epoch(model, loader, optimizer=None) -> float:
    ...
    for x, y, _ in loader:
        x = x[None, :, :, :].to(DEVICE)  # [1, 1, F, T]
        y = y[None, :, :, :].to(DEVICE)

        x_padded, original_shape = dl.pad_to_multiple_tensor(x, multiple=2 ** DEPTH)
        logits = model(x_padded)
        logits = dl.crop_tensor_to_shape(logits, original_shape)

        loss = dl.bce_dice_loss(
            logits,
            y,
...
def collect_validation_probabilities(model, files: List[pathlib.Path]) -> List[Tuple[np.ndarray, np.ndarray, str]]:
    ...
    for mix_path in files:
        ...
        data, sr = dl.load_audio(mix_path, resampled_sr=RESAMPLED_SR)
        _, _, Sxx = dl.compute_spectrogram(data, sr, nperseg=NPERSEG, noverlap=NOVERLAP)
        prob = dl.predict_probability_map_from_sxx(Sxx, model=model, device=DEVICE)
...
def find_best_threshold(validation_outputs: List[Tuple[np.ndarray, np.ndarray, str]]) -> Tuple[float, Dict[str, float]]:
    ...
    for threshold in THRESHOLD_GRID:
        ...
        if best_metrics is None or mean_metrics["f1"] > best_metrics["f1"]:
...
def main():
    ...
    metadata = {
        "method": "pure_dl_spectrogram_unet",
        "in_channels": 1,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if improved:
            ...
            dl.save_checkpoint(CHECKPOINT_PATH, model, metadata=metadata)
        else:
            bad_epochs += 1
    ...
    best_model, best_metadata = dl.load_checkpoint(CHECKPOINT_PATH, device=DEVICE)
    ...
    dl.save_checkpoint(CHECKPOINT_PATH, best_model, metadata=best_metadata)

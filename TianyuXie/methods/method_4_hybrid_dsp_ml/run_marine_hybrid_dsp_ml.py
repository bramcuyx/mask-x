...
def main():
    ...
    masks_folder = pathlib.Path(config["paths"]["masks_folder"]) / "hybrid_dsp_ml"
    prob_folder = pathlib.Path(config["paths"]["masks_folder"]) / "hybrid_dsp_ml_prob"
    raw_prob_folder = pathlib.Path(config["paths"]["masks_folder"]) / "hybrid_dsp_ml_raw_prob"
    plots_folder = pathlib.Path(config["paths"]["plots_folder"]) / "hybrid_dsp_ml"

    if args.model is not None:
        model_path = pathlib.Path(args.model)
    else:
        configured = config.get("paths", {}).get("hybrid_dsp_ml_model_bundle", None)
        if configured is None:
            raise ValueError(
                "No model path provided. Run with --model PATH_TO_PKL, or add "
                "paths.hybrid_dsp_ml_model_bundle to mask_config.yaml."
            )

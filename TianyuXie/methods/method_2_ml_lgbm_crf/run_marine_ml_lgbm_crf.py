"""
Run Method 2: Pure ML LightGBM + optional lightweight 2D CRF on marine wav files.

Usage examples:
    python run_marine_ml_lgbm_crf.py --model "D:/Fuss_For_ICT/.../ssdata/train/fuss_ml_lgbm_crf_model_all.pkl"

Optional:
    Add this to mask_config.yaml:

    paths:
      ml_model_bundle: "D:/Fuss_For_ICT/.../ssdata/train/fuss_ml_lgbm_crf_model_all.pkl"

Then you can run:
    python run_marine_ml_lgbm_crf.py
"""
...
def plot_ml_result(Sxx, mask, raw_prob, decoded_prob, f, t, out_png: pathlib.Path):
    ...
    ax[2].set_title("Decoded Probability")
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the .pkl model bundle saved by evaluate_fuss_ml_lgbm_crf.py.",
    )
...
def main():
    ...
    masks_folder = pathlib.Path(config["paths"]["masks_folder"]) / "ml_lgbm_crf"
    prob_folder = pathlib.Path(config["paths"]["masks_folder"]) / "ml_lgbm_crf_prob"
    raw_prob_folder = pathlib.Path(config["paths"]["masks_folder"]) / "ml_lgbm_raw_prob"
    plots_folder = pathlib.Path(config["paths"]["plots_folder"]) / "ml_lgbm_crf"

    if args.model is not None:
        model_path = pathlib.Path(args.model)
    else:
        configured = config.get("paths", {}).get("ml_model_bundle", None)
        if configured is None:
            raise ValueError(
                "No model path provided. Run with --model PATH_TO_PKL, or add "
                "paths.ml_model_bundle to mask_config.yaml."
            )
    ...
    payload = ml.load_model_bundle(model_path)
    model = payload["model"]
    ...
    for i, wav_path in enumerate(event_files, start=1):
        ...
        mask, raw_prob, decoded_prob, Sxx, f, t = ml.estimate_mask_file(
            wav_path,
            model=model,
            nperseg=nperseg,
            noverlap=noverlap,
        ...
        mask, cleaned = enforce_marine_padding(
            ...
            t,
            padding=padding,
            probability_maps=[raw_prob, decoded_prob],
        )

        raw_prob, decoded_prob = cleaned
        ...
        plot_ml_result(Sxx, mask, raw_prob, decoded_prob, f, t, plots_folder / f"{wav_path.stem}.png")
